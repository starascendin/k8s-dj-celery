...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        content_orchestrator = ContentPromptOrchestrator(prompt_service=prompt_service, content_service=content_service)

        content: ContentSource = ContentSource.objects.get(id="92beee35-2d45-4f94-9612-9a9df9e965ce")
        chunks = content_service.chunk_yt_processed(content.contentprocessed_set.first())
        
        # TODO: below needs to be refactored
        results = content_orchestrator._apply_prompts_to_chunks(chunks)
        
        
        # print("# run prompting results", results)
        # completions = content_service.get_content_chunks_prompt_completions(content)

        # all_chunks = content.ContentProcessed_set.first().ContentYtChunk_set.all()
        # chunks = content
        # chunks = content_service.chunk_yt_content(content)
